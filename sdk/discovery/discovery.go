...
type Request = market.DiscoveryRequest
...
func (e *Engine) Discover(ctx context.Context, req market.DiscoveryRequest) ([]*market.Pool, map[string]any, error) {
	if req.Mint.IsZero() {
...
func RankPools(pools []*market.Pool, targetMint string, quoteMint *string) []*market.Pool {
	...
	score := func(p *market.Pool) {
		...
		w4 := decimal.NewFromFloat(0.15)
		w5 := decimal.NewFromFloat(0.10)
		...
		normTVL := normLiq
		normVol := decimal.Zero
		if v, ok := p.Metadata["volume_24h"].(float64); ok {
			normVol = decimal.NewFromFloat(v).Div(decimal.NewFromFloat(1_000_000))
			if normVol.GreaterThan(decimal.NewFromInt(1)) {
				normVol = decimal.NewFromInt(1)
			}
		}
		fresh := decimal.NewFromFloat(1)
		if time.Since(p.LastUpdatedAt) > 30*time.Minute {
			fresh = decimal.NewFromFloat(0.3)
		}
